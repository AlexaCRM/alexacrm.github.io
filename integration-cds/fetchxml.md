...
- Alias the `link-entity`. You will be able to access linked table columns via dot-notation: {% raw %}`{{record["aliasedEntity.attributeName"]}}`{% endraw %}.
- Alias the columns inside `link-entity`. You will be able to access the aliased columns directly: {% raw %}`{{record["aliasedLinkedAttributeName"]}}`{% endraw %}. Keep in mind that naming collisions may occur between alias names and main table column names.
